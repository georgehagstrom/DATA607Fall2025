...
-   We will use the `TTR` package (which is part of the `tidyquant` family of packages, [see here](https://cran.r-project.org/web/packages/tidyquant/refman/tidyquant.html)). The main function we will use is called `runMean`. An alternative package that is also very nice but not part of the `tidyverse` is [`RcppRoll`](https://CRAN.R-project.org/package=RcppRoll)
